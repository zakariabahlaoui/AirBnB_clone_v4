$(function () {
    ...
    success: function (response) {
      response.forEach(element => {
        ...
        <article>
          ...
          <div class=reviews>
              <h2>Reviews</h2>
              <span class="fetch-reviews" data-review-id="${element.id}">Show</span>
              <hr>
              <ul></ul>
          </div>
			  </article>
  ...
  $(document).on('click', '.fetch-reviews', function () {
    ...
    reviewsContainer = $(this).parent().find('ul');
    if ($(this).text() == 'Show') {
      $(this).text('Hide');

      // call reviews API to get reviews list
      reviews_endpoint = `${apiBaseURL}/api/v1/places/${reviewId}/reviews`;
      $.get(reviews_endpoint, function (reviews) {
        reviews.forEach(review => {
          userId = review.user_id;
          // call users API to get user
          user_endpoint = `${apiBaseURL}/api/v1/users/${userId}`;
          $.get(user_endpoint, function (user) {
            // display reviews
